---
title: "Forecast of the 2024 Presidential Election through the Lens of Turbulent Swing States"
subtitle: "Kamala Harris Leads the Race With 276 Electoral Votes With Support in 4 of 7 Swing States"
author: 
  - Andy Jiang
  - Onon Burentuvshin
  - Rui Hu
...
library(bayesplot)
library(rstanarm)
data_filtered <- read.csv(here::here("data/02-analysis_data/model_data.csv"))
data <- read_parquet(here::here("data/02-analysis_data/analysis_data.parquet"))
```


# Introduction
As the 2024 U.S. presidential election approaches, the race between Kamala Harris and Donald Trump reflects a deeply polarized electorate, particularly within key swing states. In an era where public opinion is continually shaped by a fast-paced media cycle and partisan divides, accurate election forecasting has never been more challenging or crucial. Swing states, where voter sentiment can tip the balance of the electoral college, have garnered particular attention. Traditional polls provide a snapshot of public opinion, but varying methodologies and sample biases limit their reliability. Thus, this paper seeks to bridge the gap in predictive accuracy by employing an aggregated “poll-of-polls” approach focused on capturing nuanced insights into voter behavior within these pivotal states.

This study builds a predictive model using aggregated polling data from a selection of high-quality pollsters, each rated above a minimum quality threshold. By concentrating on states historically known to sway election outcomes—such as Pennsylvania, Wisconsin, Nevada, and Michigan—this approach aims to provide a more resilient forecast against individual poll biases. The model examines demographic influences, candidate-specific factors, and recent endorsements, all while accounting for each state’s unique political landscape. Additionally, this paper critiques traditional polling methods, suggesting improvements in sampling and weighting techniques to achieve greater external validity. Given the substantial financial and temporal constraints inherent in polling, we propose an idealized methodology to enhance forecasting accuracy within a manageable budget of $100,000.

The results of this analysis reveal a tentative lead for Kamala Harris, projecting 276 electoral votes in her favor, slightly above the 270 required to win. While she holds narrow but consistent margins in four critical swing states, her lead remains precarious in light of late-stage shifts in voter sentiment. These results emphasize the importance of demographic factors—such as Harris’s appeal among younger and immigrant voters—as well as the impact of recent high-profile endorsements for Trump, which could mobilize his base. The analysis suggests that the 2024 election will likely hinge on voter turnout in these contested states, highlighting the significance of demographic-targeted campaigns and last-minute electoral efforts in shaping the final outcome.

In the **Data** section (see [Section 2](#sec-data)), we describe the sources and processing steps involved in preparing our dataset. Following that, the **Model** section (see [Section 3](#sec-model)) outlines the statistical model used to analyze the data and make predictions. 

The **Results** section (see [Section 4](#sec-result)) provides an overview of the model's forecasts and key findings, while the **Discussion** (see [Section 5](#sec-discussion)) interprets these results and considers broader implications. For additional technical information, including detailed model specifications, refer to the **Model Details** subsection (see [Section 5.1](#sec-model-details)).
...
## Predictor variables
End date numeric:
This variable represents the numeric conversion of the poll end date, indicating the time at which the polling data was collected. By transforming dates into numeric values, we can account for temporal trends and shifts in voter sentiment. This predictor helps identify changes in support over time and captures any momentum shifts that occur as the election draws closer.

Candidate name:
This categorical variable distinguishes between the main candidates: Donald Trump and Kamala Harris. By including candidate-specific factors, the model can account for differences in support levels attributed to individual characteristics or public perception of each candidate. Additionally, this variable enables the model to gauge the relative standing between the two contenders in different states and overall.

# Model {#sec-model}
In this analysis, we utilize a logistic regression model to predict polling percentages for the 2024 U.S. presidential candidates over time. In order to focus on states with historical significance in elections. The model is specified as a Generalized Linear Model (GLM) with a binomial family and a logit link function, appropriate for modeling binary or proportion-based outcomes.  The United State's electoral college system operates under a winner takes all votes mechanic where all the electoral votes are allocated to whichever candidate receives the larger share of votes within that state. Therefore, our model considers states outside the realm of swing states to have allocated a certain number of electoral votes to each candidate wherein Donald Trump has 219 seats and Kamala Harrris has  226. This initial distribution of electoral votes is provided by New York Times' tracking of swing states [@boyce_swing_states]. Our model will produce projected shares in each state from which the electoral votes of swing states will be allocated to each  candidate to predict the winner of the presidential election.
...
@fig-4 shows the trend of support rate changes for each candidate before the upcoming election, as well as the model's predicted values for the next 20 days.We predicts that Kamala Harris may win based on our model.
However, this shows the trend of support rate changes for each candidate before the upcoming election, as well as the model's predicted values for the days until the election. This support rate changes model exhibits a higher rate of growth in Donald Trump's share of support. Although Kamala Harris is in the lead, Donald Trump's growth of support rate is an element that could stifle Harris' lead and drop her below her required 270 electoral votes. 
...
ggplot(all_data, aes(x = end_date, y = pct, color = candidate_name)) +
  ...
  scale_x_date(date_labels = "%b", date_breaks = "1 month", 
               limits = as.Date(c("2024-01-01", "2024-11-5"))) +
  scale_y_continuous(labels = percent_format(), limits = c(0.44, 0.47))
...
## Turbulent Margins
Based on our results from @fig-1 predicted that Kamala Harris to win the presidential election with 276 electoral votes. Furthermore, @fig-2 showed that her lead in the states of Pennsylvania, Wisconsin, Nevada, and Michigan are stronger than Donald Trump's support in Pennsylvania, Wisconsin, Nevada, and Michigan. From @fig-2, we can also observe that Kamala Harris' lead in Nevada seems to be the weakest. In the framework of the electoral college system, losing Nevada to Donald Trump would not change the results as Nevada holds 6 electoral votes, which means that Kamala Harris would still possess the necessary 270 to win the election. However, this is not the case if Kamala Harris loses support in any of the three other states of Pennsylvania, Wisconsin, and Michigan as this would place her beneath the 270 seat threshold. In fact, losing any one of these states would result in Donald Trump's victory. @fig-3 showed that the support rate of Donald Trump is increasing faster than that of Kamala Harris', threatening Harris' lead over Pennsylvania, Wisconsin, and Michigan. 
...
## Sampling Methodology Employed by New York Times and Siena College {#sec-Methodology}
...
###   Referral Mechanics
To further broaden our reach without exponentially increasing advertising costs, we incorporate a referral system that rewards respondents for inviting others to complete the survey [@tellingstories]. Each referral allows the original respondent to gain an additional entry in the lottery, increasing their chances of winning. This referral incentive taps into snowball sampling, where respondents refer people within their network to participate, expanding our pool of respondents organically.
...
A copy of the PDF of our survey can be found in other/datasheet/Poll Survey - Google Forms.pdf
...
@fig-stanareyouokay-1 is a trace plot. It shows chain mixing, stationarity and no drifting. This suggests that the model's MCMC sampling achieved convergence for each parameter. The chains are well-mixed and stationary, providing confidence in the posterior estimates derived from this sampling process. 

@fig-stanareyouokay-2 is a Rhat plot. It shows the Rhat values for the parameters estimated in the model. Values close to 1 indicate that the chains have mixed well and the posterior distribution has been adequately explored. This suggests that the model has converged successfully and the estimates are reliable for inference.
